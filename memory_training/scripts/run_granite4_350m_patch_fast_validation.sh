...
run_id="granite4-350m-patch-multitask-noop5-fastval-grouped-v2-v1-10-e${epochs}-b8-trainseed${training_seed}-${run_tag}"
...
"${python_bin}" -m memory_training.train \
  ...
  --closed-loop-full-scenarios 83 111 \
  --closed-loop-sparse-scenarios 82 \
  --closed-loop-scenario-batch-size 2 \
  --closed-loop-quiz \
  --closed-loop-sparse-noop-keep-fraction 0.10 \
  --false-update-threshold 0.20

exec bash "${repo_root}/memory_training/scripts/run_granite4_350m_fastval_post.sh" \
  "${gpu}" "${run_id}"
