...
validate() {
  ...
  "${python}" -m memory_training.validate_hf_checkpoint_v2 \
    ...
    --closed-loop-only --scenario-batch-size 3 --quiz-batch-size 16 \
    --no-status-update >"${log}" 2>&1
}

summary=granite4-1b-summary-multitask-noop5-trainfirst-grouped-v2-v1-10-e4-b8-trainseed45-r1
patch=granite4-1b-patch-multitask-noop5-trainfirst-grouped-v2-v1-10-e4-b8-trainseed45-r1
delta=granite4-1b-delta-v3-multitask-noop5-trainfirst-grouped-v2-v1-10-e4-b8-trainseed45-r1

# Start 1B Summary as soon as the first 350M wave completes and 350M Summary begins.
while true; do
  completed=$(find "${workspace}/runs" -maxdepth 2 -type f -name 'eval-fixed-v2-validation-best-epoch-*.json' -path '*/granite4-350m-*' | wc -l)
  (( completed >= 2 )) && break
  tmux has-session -t granite4-350m-new-fixed-validations-gpu5 2>/dev/null || exit 1
  sleep 10
done
