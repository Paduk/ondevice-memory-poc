...
train_root="${data_parent}/grouped-s1-s100-plus-temporal-t1-t20-plus-v1-10-v2-delta-v3-compact-k${interval}-v1"
...
run_id="qwen35-2b-${method}-multitask-noop5-uniform-depth-e${epochs}-b2-trainseed${training_seed}-evalfixed-noop5-${run_tag}"
...
if [[ ! -d "${final_checkpoint}/adapter" ]]; then
  echo "[$(date -Is)] Starting Qwen3.5 2B ${method} training on GPU ${gpu}."
...
echo "[$(date -Is)] Starting fixed validation for epochs 3 and 4."
validate_epoch 3 & p3=$!
validate_epoch 4 & p4=$!
failed=0
wait "${p3}" || failed=1
...
output.write_text(
    json.dumps(
        {"selection_metric": "composite_60_25_15", "evaluated_epochs": [3, 4],
         "results": rows, "winner": winner},
        indent=2,
    )
    + "\n"
)
