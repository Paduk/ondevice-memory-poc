...
output_root="${workspace}/benchmarks/kv-cache-qwen35-0.8b-stress-validation-once-${run_tag}"
...
run_method() {
  ...
  for update_count in 20 40 60 80; do
    ...
    output_dir="${output_root}/update${update_count}/${method}/on"
    if [[ -s "${output_dir}/summary.json" ]]; then
      echo "[$(date -Is)] reuse update${update_count}/${method}/on"
      continue
    fi
...
if [[ "$status" -ne 0 ]]; then
  echo "One or more stress benchmark workers failed: ${output_root}" >&2
  exit "$status"
fi
