...
output_root="${workspace}/benchmarks/granite4-1b-stress-test-once-${run_tag}"
...
k2_run="${workspace}/runs/granite4-1b-delta_v3_compact_k2-multitask-noop5-uniform-depth-e4-b2-trainseed45-r1"
k5_run="${workspace}/runs/granite4-1b-delta_v3_compact_k5-multitask-noop5-uniform-depth-e4-b2-trainseed45-r1"
k10_run="${workspace}/runs/granite4-1b-delta_v3_compact_k10-multitask-noop5-uniform-depth-e4-b2-trainseed45-r1"

resolve_winner() {
  local run_dir=$1
  local selection="${run_dir}/eval-fixed-best-checkpoint.json"
  if [[ ! -s "$selection" ]]; then
    echo "Best-checkpoint selection not found: ${selection}" >&2
    return 2
  fi
  jq -er '.winner.checkpoint' "$selection"
}

k2_checkpoint=$(resolve_winner "$k2_run")
k5_checkpoint=$(resolve_winner "$k5_run")
k10_checkpoint=$(resolve_winner "$k10_run")
...
run_method 2 patch "$patch_checkpoint" >"${log_root}/patch.log" 2>&1 &
patch_pid=$!
run_method 3 delta_v3_compact_k2 "$k2_checkpoint" >"${log_root}/k2.log" 2>&1 &
k2_pid=$!
run_method 4 delta_v3_compact_k5 "$k5_checkpoint" >"${log_root}/k5.log" 2>&1 &
k5_pid=$!
run_method 6 delta_v3_compact_k10 "$k10_checkpoint" >"${log_root}/k10.log" 2>&1 &
k10_pid=$!
...
if [[ "$status" -ne 0 ]]; then
  echo "One or more Granite stress Test workers failed: ${output_root}" >&2
  exit "$status"
fi
