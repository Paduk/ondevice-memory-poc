...
validate() {
  ...
  output="${run_dir}/eval-fixed-validation-sampling-t07-p09-seed777-best-${epoch}.json"
  log="${run_dir}/eval-fixed-validation-sampling-t07-p09-seed777-best-${epoch}.log"
  "${python}" -m memory_training.validate_hf_checkpoint_v2 \
    ...
    --evaluation-seed 777 --do-sample --temperature 0.7 --top-p 0.9 \
