...
const state = { overview: null, selected: new Set(), comparison: null };
...
const badge = (run) => `<span class="badge ${run.state.toLowerCase()} ${run.stale?'stale':''}">${esc(run.stale?'STALE':run.state)}</span>`;
...
async function refresh() {
  try {
    state.overview = await api("/api/runs");
    renderAll();
...
function renderAll() {
  ...
  renderStats(); renderMatrix(); renderRuns(); renderSelection();
}
...
function renderStats() {
  ...
  const cards = [
    ...
    ["Running", o.counts.RUNNING, o.stale_count ? `${o.stale_count} stale heartbeat` : "live processes"],
...
function metricSeries(run, metricKey) {
  ...
  const event = eventName === "train" ? "train" : "validation_epoch";
  return run.metrics.filter(m=>m.event===event && m[key]!==undefined).map(m=>({x:Number(m.global_step??m.epoch??0),y:Number(m[key])}));
}
