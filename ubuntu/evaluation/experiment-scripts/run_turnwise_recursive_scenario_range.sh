...
readonly START_SCENARIO="${3:?start scenario is required}"
readonly SCENARIO_LIMIT="${4:?scenario limit is required}"
readonly RANGE_SLUG="${5:?range slug is required}"
...
readonly EXPERIMENT_ROOT="${ARTIFACT_ROOT}/evaluation/vehiclemembench/turnwise-${SLUG}-${RANGE_SLUG}-20260813"
...
if [[ -e "${EXPERIMENT_ROOT}" || -e "${CACHE_ROOT}" ]]; then
  echo "Fresh experiment or cache root already exists for ${SLUG}/${RANGE_SLUG}" >&2
  exit 1
fi
...
echo "[$(date --iso-8601=seconds)] ${PROFILE} ${RANGE_SLUG} started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] ${PROFILE} ${RANGE_SLUG} completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] ${PROFILE} ${RANGE_SLUG} failed status=${status}" >&2
fi
