...
readonly EXPERIMENT_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench/turnwise-summary-fresh-r2-s${SCENARIO}-20260815"
...
echo "[$(date --iso-8601=seconds)] Turn-wise Recursive Summary Scenario ${SCENARIO} R2 started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] Scenario ${SCENARIO} completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] Scenario ${SCENARIO} failed status=${status}" >&2
fi
