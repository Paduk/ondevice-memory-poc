...
readonly EXPERIMENT_ROOT="${ARTIFACT_ROOT}/evaluation/vehiclemembench/recursive-summary-patch-v4-repair-s1-s10-20260812"
...
echo "[$(date --iso-8601=seconds)] v4 prompt run/resume started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] v4 prompt experiment completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] v4 prompt experiment failed status=${status}" >&2
fi
exit "${status}"
