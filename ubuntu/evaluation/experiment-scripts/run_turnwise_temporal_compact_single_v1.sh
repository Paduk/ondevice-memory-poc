...
readonly RUN_LABEL="${2:-v1-20260815}"
...
readonly EXPERIMENT_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench/turnwise-temporal-compact-s${SCENARIO}-${RUN_LABEL}"
...
echo "[$(date --iso-8601=seconds)] Temporal+Compact Scenario ${SCENARIO} started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] Temporal+Compact Scenario ${SCENARIO} completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] Temporal+Compact Scenario ${SCENARIO} failed status=${status}" >&2
fi
