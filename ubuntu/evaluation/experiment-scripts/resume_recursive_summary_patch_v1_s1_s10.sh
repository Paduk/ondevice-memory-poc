...
readonly EXPERIMENT_ROOT="${ARTIFACT_ROOT}/evaluation/vehiclemembench/recursive-summary-patch-v1-repair-s1-s10-20260812"
...
echo "[$(date --iso-8601=seconds)] v1 prompt + repair fresh run started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] v1 prompt experiment completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] v1 prompt experiment failed status=${status}" >&2
fi
exit "${status}"
