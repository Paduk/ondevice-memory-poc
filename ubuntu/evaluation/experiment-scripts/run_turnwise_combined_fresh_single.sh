...
readonly EXPERIMENT_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench/turnwise-combined-fresh-r${REPETITION}-s${SCENARIO}-20260815"

if (( SCENARIO < 11 || SCENARIO > 50 )); then
  echo "Scenario must be between 11 and 50" >&2
  exit 2
fi
...
echo "[$(date --iso-8601=seconds)] Combined Scenario ${SCENARIO} R${REPETITION} started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] Combined Scenario ${SCENARIO} R${REPETITION} completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] Combined Scenario ${SCENARIO} R${REPETITION} failed status=${status}" >&2
fi
