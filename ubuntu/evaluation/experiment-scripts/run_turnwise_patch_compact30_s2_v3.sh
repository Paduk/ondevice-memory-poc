...
readonly EXPERIMENT_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench/turnwise-patch-compact30-s2-v3-20260814"
...
echo "[$(date --iso-8601=seconds)] compact30 Scenario 2 run started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] compact30 Scenario 2 run completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] compact30 Scenario 2 failed status=${status}" >&2
fi
