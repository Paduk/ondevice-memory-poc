...
readonly EXPERIMENT_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench/turnwise-temporal-patch-s6-s10-v1-20260814"
...
echo "[$(date --iso-8601=seconds)] Temporal Patch Scenarios 6-10 run started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] Temporal Patch Scenarios 6-10 completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] Temporal Patch Scenarios 6-10 failed status=${status}" >&2
fi
