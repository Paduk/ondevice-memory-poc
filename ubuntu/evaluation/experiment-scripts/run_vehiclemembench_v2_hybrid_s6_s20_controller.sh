...
readonly RUN_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench-v2-hybrid/hybrid-scale-s06-s20-r1"
...
if (($#)); then
  ...
  scenario_indexes=($(seq 6 20))
fi
for scenario_index in "${scenario_indexes[@]}"; do
  if ((scenario_index < 6 || scenario_index > 20)); then
    printf 'scenario index must be in [6, 20]: %s\n' "$scenario_index" >&2
    exit 2
  fi
done

printf '[%s] Hybrid controller started (parallel=%d scenarios=%s)\n' \
...
completed_count="$(find "$STATUS_ROOT" -type f -name 'scenario-*.status' -exec grep -l '^COMPLETED' {} + | wc -l)"
failed_count="$(find "$STATUS_ROOT" -type f -name 'scenario-*.status' -exec grep -l '^FAILED' {} + | wc -l)"
