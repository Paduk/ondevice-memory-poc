...
readonly EXPERIMENT_ROOT="/mnt/data/hj153lee/PalmClaw/evaluation/vehiclemembench/turnwise-temporal-compact-s6-v1-20260814"
...
echo "[$(date --iso-8601=seconds)] Temporal+Compact Scenario 6 started"
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] Temporal+Compact Scenario 6 completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] Temporal+Compact Scenario 6 failed status=${status}" >&2
fi
