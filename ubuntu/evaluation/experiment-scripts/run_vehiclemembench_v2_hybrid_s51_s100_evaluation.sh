...
readonly RECOVERY_SESSION="v2-hybrid-recover-s99"

scenarios=($(seq 51 100))

printf '[%s] Waiting for S51-S100 generation recovery\n' "$(date -u +%FT%TZ)"
...
if [[ "$completed_count" -ne 50 ]]; then
  printf '[%s] Evaluation aborted: only %s/50 scenarios completed\n' \
...
printf '[%s] Hybrid state-evolution S51-S100 evaluation completed\n' \
  "$(date -u +%FT%TZ)"
