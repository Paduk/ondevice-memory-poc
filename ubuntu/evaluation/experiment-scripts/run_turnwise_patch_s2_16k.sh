...
readonly EXPERIMENT_ROOT="${ARTIFACT_ROOT}/evaluation/vehiclemembench/turnwise-patch-s2-16k-20260814"
...
if [[ -e "${EXPERIMENT_ROOT}" || -e "${CACHE_ROOT}" ]]; then
  echo "Fresh Scenario 2 16K experiment or cache already exists" >&2
  exit 1
fi
...
echo "[$(date --iso-8601=seconds)] Turn-wise Patch Scenario 2 16K started"
...
"${PALMCLAW_BIN}" eval vehicle \
  ...
  --mode live \
  --profiles cloud_turnwise_recursive_summary_patch \
  --scenario 2 \
  --task-limit 10 \
  --max-tool-rounds 10 \
  --model gpt-5.6-terra \
  ...
  --recursive-summary-max-memory-chars 16384 \
...
if [[ ${status} -eq 0 ]]; then
  ...
  echo "[$(date --iso-8601=seconds)] Turn-wise Patch Scenario 2 16K completed"
else
  printf '%s\n' "${status}" > "${EXPERIMENT_ROOT}/FAILED"
  echo "[$(date --iso-8601=seconds)] Turn-wise Patch Scenario 2 16K failed status=${status}" >&2
fi
